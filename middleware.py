...
class Middleware:
    ...
    def search(self, search_queries: list[str], num_results):
        ...
        for query in search_queries:
            ...
            query_vec, query_embedding_time = colpali_manager.process_text([query])
            query_vec = query_vec[0]
            search_res = self.milvus_manager.search(query_vec, topk=num_results)
            print(f"Search result: {search_res} for query: {query}")

            # Calculate time taken
            total_retrieval_time = sum(result[0] for result in search_res)

        return search_res, total_retrieval_time, query_embedding_time
